...
def _as_text(value, default=MISSING):
    """Return a string representation; *default* for empty/missing."""
    if value in (None, ''):
        return default
...
def _escaped(value, default=MISSING):
    """HTML-escape the result of _as_text."""
    return escape(_as_text(value, default))


def _styles():
    """Create all paragraph styles with word wrapping disabled."""
...
def _boxed_table(rows, widths, row_heights=None, font_size=9.9, padding=4.0, header_rows=0):
    """Consistently styled table with alternating backgrounds."""
...
def _field_paragraph(label, value, styles):
    """Label + value pair; value always filled (dash if missing)."""
    text = (
        f'<font size="9.6" color="#1d4ed8"><b>{escape(label)}</b></font><br/>'
        f'<font size="10.7" color="#0f172a">{_escaped(value)}</font>'
    )
    return Paragraph(text, styles['field'])


def _single_row_table(pairs, styles):
    """Build a two‑column table from label‑value pairs (padded to even count)."""
    rows = []
    if len(pairs) % 2:
        pairs = list(pairs) + [('', '')]
    for i in range(0, len(pairs), 2):
        left = _field_paragraph(pairs[i][0], pairs[i][1], styles)
        right = _field_paragraph(pairs[i + 1][0], pairs[i + 1][1], styles)
...
def _header(story, styles):
    ...
    story.append(
        Paragraph(
            'Address: Plot 8, Block 46, Kijitonyama, Mpakani Centre, 3rd Floor, Suite F1.01, Dar es Salaam',
            styles['tiny_center'],
        )
    )
    ...
    logo = (
        Image(str(logo_path), width=30 * mm, height=22 * mm)
        if logo_path.exists()
        else ''
    )
    office_text = (
    ...
    header_table = Table(
        [[Paragraph(office_text, styles['office']), logo]],
        colWidths=[150 * mm, 34 * mm],
    )
    ...
    story.append(Spacer(1, 2 * mm))
    story.append(
        HRFlowable(width='100%', thickness=1.2, color=BRAND_ORANGE, spaceBefore=0, spaceAfter=0)
    )
...
def _footer(canvas, doc):
    ...
    canvas.drawString(14 * mm, 10 * mm, f'Application ID: {getattr(doc, "serial_number", MISSING)}')
    canvas.drawCentredString(105 * mm, 10 * mm, f'Page {canvas.getPageNumber()} of {TOTAL_PAGES}')
    canvas.drawRightString(196 * mm, 10 * mm, f'Generated: {getattr(doc, "generated_date", MISSING)}')
    canvas.restoreState()


def _photo_flowable(student_profile, styles):
    """Return a framed image or placeholder for the student photo."""
    ...
    placeholder = Table(
        [[Paragraph('Student Photo', styles['body'])]],
        colWidths=[36 * mm],
        rowHeights=[44 * mm],
    )
...
def build_csc_style_application_pdf(application, student_profile=None, supplemental_profile=None):
    """
    Return an HttpResponse containing a 6‑page professional application form.
    Every field has its own dedicated space – no concatenation, no blank cells.
    """
    styles = _styles()

    response = HttpResponse(content_type='application/pdf')
    filename = f'application_{application.id}_{application.student.username}.pdf'
    ...
    generated_date = (
        ...
        else generated_source.strftime('%d/%m/%Y')
    )
    ...
    # Student details
    student_name = (
        getattr(supplemental_profile, 'full_name_passport', None)
        or application.student.get_full_name()
        or application.student.username
    )
    gender = (
        student_profile.get_gender_display()
        if student_profile and getattr(student_profile, 'gender', None)
        else ''
    )
    work_experiences = (
        list(WorkExperience.objects.filter(student=student_profile).order_by('-start_date'))
        if student_profile
        else []
    )
    ...
    # Each field is a separate label‑value pair – no concatenation
    personal_fields = [
        ('Full Name (as in passport)', student_name),
        (
            'Gender',
            f'{_bool_box(gender == "Male")} Male   {_bool_box(gender == "Female")} Female',
        ),
        ...
        ('Form Six School Name', getattr(student_profile, 'alevel_school', None) if student_profile else None),
        ('Form Six School Address', getattr(student_profile, 'alevel_school_street', None) if student_profile else None),
    ...
    page_one_data = [
        _single_row_table(personal_fields, styles),
        _photo_flowable(student_profile, styles),
    ]
    page_one = Table(
        [[page_one_data[0], page_one_data[1]]],
    ...
    story.append(
        _single_row_table(
            [
                ...
                ('Address (Street/Village/House No.)', getattr(student_profile, 'emergency_street', None) if student_profile else None),
            ],
            styles,
        )
    )
        ...
        _boxed_table(
            [
                ...
                [
                    ...
                    Paragraph(_escaped(getattr(student_profile, 'olevel_school', None)), styles['body']),
                    Paragraph(_escaped(getattr(student_profile, 'olevel_candidate_no', None)), styles['body']),
                    Paragraph(_escaped(getattr(student_profile, 'olevel_completed_year', None)), styles['body']),
                    Paragraph(_escaped(getattr(student_profile, 'olevel_gpa', None)), styles['body']),
                ],
                [
                    Paragraph('Form Six (A-Level)', styles['body']),
                    Paragraph(_escaped(getattr(student_profile, 'alevel_school', None)), styles['body']),
                    Paragraph(_escaped(getattr(student_profile, 'alevel_candidate_no', None)), styles['body']),
                    Paragraph(_escaped(getattr(student_profile, 'alevel_completed_year', None)), styles['body']),
                    Paragraph(_escaped(getattr(student_profile, 'alevel_gpa', None)), styles['body']),
                ],
            ],
    ...
    story.append(
        _boxed_table(
            [
                [
                    Paragraph('<b>Level</b>', styles['body']),
                    Paragraph('<b>Institution</b>', styles['body']),
                    Paragraph('<b>Field of Study</b>', styles['body']),
                    Paragraph('<b>Year Completed</b>', styles['body']),
                    Paragraph('<b>GPA</b>', styles['body']),
                ],
                [
                    Paragraph('Certificate', styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'certificate_institution', None)), styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'certificate_field_of_study', None)), styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'certificate_year_completed', None)), styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'certificate_gpa', None)), styles['body']),
                ],
                [
                    Paragraph('Diploma', styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'diploma_institution', None)), styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'diploma_field_of_study', None)), styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'diploma_year_completed', None)), styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'diploma_gpa', None)), styles['body']),
                ],
                [
                    Paragraph('Bachelor Degree', styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'bachelor_institution', None)), styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'bachelor_field_of_study', None)), styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'bachelor_year_completed', None)), styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'bachelor_gpa', None)), styles['body']),
                ],
                [
                    Paragraph('Master Degree', styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'master_institution', None)), styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'master_field_of_study', None)), styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'master_year_completed', None)), styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'master_gpa', None)), styles['body']),
                ],
                [
                    Paragraph('PhD', styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'phd_institution', None)), styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'phd_field_of_study', None)), styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'phd_year_completed', None)), styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'phd_gpa', None)), styles['body']),
                ],
            ],
    ...
    story.append(
        _boxed_table(
            [[Paragraph(_escaped(getattr(supplemental_profile, 'professional_qualifications', None)), styles['body'])]],
            [184 * mm],
            row_heights=[18 * mm],
        )
    )
    ...
    story.append(
        _boxed_table(
            [
                ...
                [
                    Paragraph(_escaped(getattr(supplemental_profile, 'english_test_name', None)), styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'english_test_score', None)), styles['body']),
                    Paragraph(_escaped(getattr(supplemental_profile, 'english_test_year', None)), styles['body']),
                ],
            ],
            [84 * mm, 50 * mm, 50 * mm],
            header_rows=1,
    ...
    story.append(
        _single_row_table(
            [
                ('Current Address (Street/Village/House No.)',
                 getattr(supplemental_profile, 'current_address', None) or (student_profile.address if student_profile else MISSING)),
                ...
                ('Permanent Address (Street/Village/House No.)', student_profile.address if student_profile else MISSING),
                ('Permanent Region', MISSING),
                ('Permanent City', MISSING),
                ('Permanent Country', getattr(student_profile, 'nationality', None) if student_profile else MISSING),
    ...
    story.append(
        _single_row_table(
            [
                ...
                ('Valid Visa?',
                 f'{_bool_box(getattr(supplemental_profile, "has_valid_visa", None) is True)} Yes   '
                 f'{_bool_box(getattr(supplemental_profile, "has_valid_visa", None) is False)} No'),
    ...
    story.append(
        _single_row_table(
            [
                ('Source', getattr(student_profile, 'heard_about_us', None) if student_profile else None),
                ('Other (please specify)', getattr(student_profile, 'heard_about_other', None) if student_profile else None),
            ],
            styles,
        )
    )
